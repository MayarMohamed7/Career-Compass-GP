...
#tried to test on dataset o wuzzuf
import re
import unicodedata
...
def extract_skills(text):
    ...
    skill_df = pd.read_csv('dataScrappingjob_data.csv', encoding='latin-1')
...
df = pd.read_csv('dataScrappingjob_data.csv', encoding='latin-1', usecols=lambda x: x != 0)
...
def clean_skill(skill):
    if isinstance(skill, str):
        ...
        cleaned_skill = ''.join(char if unicodedata.category(char)[0] == 'L' else ' ' for char in cleaned_skill)
...
pd.DataFrame(list(skill_dictionary), columns=['Skills']).to_csv('cleaned_data.csv', index=False)

# Example usage with a user paragraph
...
# Clean the user paragraph
user_paragraph = clean_paragraph(user_paragraph)

# Process the user paragraph
